...
assert.match(sandbox.window.customCards[0].description, /1\.4\.0/);
...
assert.match(code, /const CARD_VERSION = "1\.4\.0"/);
assert.doesNotMatch(code, /Android TV Remote is not linked/);

assert.match(code, /versionAtLeast/);
assert.match(code, /backendOutdated/);
assert.match(code, /manual_ad_skip_available/);
