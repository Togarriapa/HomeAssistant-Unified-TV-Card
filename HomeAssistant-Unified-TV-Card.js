...
const LABELS = {
  en: {
    ...
    confirmPowerOff: "Turn this device off?", confirmRestart: "Restart this device?",
    autoSkipAds: "Auto-skip YouTube ads", skipNow: "Skip ad now", on: "On", off: "Off",
    adResult: "Ad-skip result", actionFailed: "Action failed",
  ...
  pt: {
    ...
    autoSkipAds: "Ignorar anúncios do YouTube", skipNow: "Ignorar anúncio agora", on: "Ativo", off: "Inativo",
    adResult: "Resultado dos anúncios", actionFailed: "A ação falhou",
...
function sourceKind(value) {
  if (value.startsWith("TV App · ")) return "tv_app";
  if (value.startsWith("Cast · ")) return "cast_app";
  if (value.startsWith("Input · ")) return "input";
  return "other";
}
function displaySource(value) {
  return String(value ?? "").replace(/^TV App · /, "").replace(/^Cast · /, "").replace(/^Input · /, "");
}
function cleanControllerTitle(value) {
  return String(value ?? "").replace(/(?:\s+Controller)+$/i, "").trim() || String(value ?? "");
}
...
class UnifiedTvCard extends HTMLElement {
  ...
  static getConfigForm() {
    return {
      schema: [
        ...
        { type: "grid", name: "", flatten: true, column_min_width: "150px", schema: [
      ...
      computeLabel: (schema) => ({
        entity: "Controller entity", name: "Card title", ad_skip_entity: "Ad-skip switch override",
        show_artwork: "Show artwork", show_remote: "Show directional remote",
        show_diagnostics: "Show diagnostics", show_favorites: "Show quick app buttons",
        show_activities: "Show activity selector", show_ad_skip: "Show YouTube ad controls",
        confirm_power_off: "Confirm power off", confirm_restart: "Confirm restart",
        seek_seconds: "Seek interval (seconds)", max_favorites: "Maximum quick app buttons",
      })[schema.name],
    };
  }
  ...
  static getStubConfig(hass) {
    const entity = Object.keys(hass?.states ?? {}).find((entityId) => {
      ...
      return entityId.startsWith("media_player.") && Array.isArray(state?.attributes?.source_list)
    ...
    return {
      ...
      confirm_power_off: true, confirm_restart: true, seek_seconds: 10, max_favorites: 6,
    };
  }
  ...
  setConfig(config) {
    ...
    this._config = {
      show_artwork: true, show_remote: true, show_diagnostics: false,
      show_favorites: true, show_activities: true, show_ad_skip: true,
      confirm_power_off: true, confirm_restart: true,
      seek_seconds: 10, max_favorites: 6, ...config,
    };
    this._signature = "";
    this._render();
  }
  ...
  getCardSize() { return this._config?.show_remote === false ? 8 : 11; }
  getGridOptions() { return { columns: 12, min_columns: 4, rows: this._config?.show_remote === false ? 8 : 11, min_rows: 5 }; }
  _language() {
  ...
  _findAdSkipEntity(state) {
    ...
    let candidates = Object.keys(states).filter((entityId) => {
      ...
      return entityId.startsWith("switch.") && candidate?.attributes?.positive_detection_only === true;
    });
    if (controllerDeviceId) {
      candidates = candidates.filter((entityId) => registry?.[entityId]?.device_id === controllerDeviceId);
    } else if (physicalDeviceId) {
      candidates = candidates.filter((entityId) => states[entityId]?.attributes?.physical_device_id === physicalDeviceId);
    }
  ...
  _signatureFor(state, adEntityId, adState) {
    ...
    return JSON.stringify([
      ...
      adState?.attributes?.last_skip_at, this._notice, this._config,
    ]);
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <ha-card><div class="card ${isOff ? "is-off" : ""}">
        <header>
          <div class="identity"><div class="title">${escapeHtml(name)}</div><div class="state health-${escapeAttribute(health)}"><span class="dot"></span>${escapeHtml(state.state)} · ${escapeHtml(health)}</div></div>
        ...
        ${this._config.show_diagnostics ? `<details class="diagnostics"><summary>${escapeHtml(labels.diagnostics)}</summary><dl><dt>${escapeHtml(labels.state)}</dt><dd>${escapeHtml(state.state)}</dd><dt>${escapeHtml(labels.health)}</dt><dd>${escapeHtml(health)}</dd><dt>${escapeHtml(labels.application)}</dt><dd>${escapeHtml(attrs.app_name || attrs.app_id || "—")}</dd><dt>${escapeHtml(labels.source)}</dt><dd>${escapeHtml(activeSource || "—")}</dd><dt>${escapeHtml(labels.volume)}</dt><dd>${volume}%${muted ? " · muted" : ""}</dd>${adState ? `<dt>${escapeHtml(labels.autoSkipAds)}</dt><dd>${escapeHtml(adEnabled ? labels.on : labels.off)}</dd><dt>${escapeHtml(labels.adResult)}</dt><dd>${escapeHtml(adState.attributes?.last_result || "—")}</dd>` : ""}<dt>${escapeHtml(labels.features)}</dt><dd>${escapeHtml(attrs.supported_features ?? 0)}</dd></dl></details>` : ""}
      </div></ha-card>`;
  ...
  _appSelectBlock(options, activeSource, labels) {
    ...
    const optionHtml = groups.map((group) => `<optgroup label="${escapeAttribute(group)}">${options.filter((option) => option.group === group).map((option) => `<option value="${escapeAttribute(option.id)}" ${option.source === activeSource ? "selected" : ""}>${escapeHtml(option.label)}</option>`).join("")}</optgroup>`).join("");
  ...
  _selectBlock(id, label, placeholder, options, activeSource, stripPrefix = true) {
    ...
    return `<label class="select-control" for="${id}"><span>${escapeHtml(label)}</span><select id="${id}"><option value="">${escapeHtml(placeholder)}</option>${options.map((option) => `<option value="${escapeAttribute(option)}" ${option === activeSource ? "selected" : ""}>${escapeHtml(stripPrefix ? displaySource(option) : option)}</option>`).join("")}</select></label>`;
  }
  ...
  _confirm(message) { return typeof window.confirm !== "function" || window.confirm(message); }
  ...
  _bindEvents(adEntityId) {
    ...
    this.shadowRoot.querySelectorAll("[data-action]").forEach((button) => button.addEventListener("click", async () => {
      ...
      if (action === "skip-ad-now") {
        ...
        if (ok) this._setNotice(this._labels().skipNow, "success");
      }
    }));
    ...
    activity?.addEventListener("change", async () => {
      ...
      await this._call("cast_attribute_sensors", "run_activity", { entity_id: entityId, activity: activity.value });
      activity.value = "";
    });
...
if (!window.customCards.some((card) => card.type === CARD_TAG)) {
  window.customCards.push({
    ...
    description: "A responsive TV, Cast, app, input, activity, ad-skip, playback and remote controller.",
    preview: true,
    ...
    getEntitySuggestion: (hass, entityId) => {
      ...
      return { config: { type: "custom:unified-tv-card", entity: entityId, show_artwork: true, show_remote: true, show_favorites: true, show_activities: true, show_ad_skip: true, confirm_power_off: true, confirm_restart: true, seek_seconds: 10 } };
    },
  });
}
