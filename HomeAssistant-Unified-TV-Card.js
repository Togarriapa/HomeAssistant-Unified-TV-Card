...
function sourceKind(value) {
  if (value.startsWith("TV App · ") || value.startsWith("Cast · ")) return "app";
  if (value.startsWith("Input · ")) return "input";
  return "other";
}
function displaySource(value) {
  return value.replace(/^TV App · /, "").replace(/^Cast · /, "").replace(/^Input · /, "");
}
...
class UnifiedTvCard extends HTMLElement {
  ...
  static getConfigForm() {
    return {
      ...
      computeLabel: (schema) => ({
        entity: "Controller entity", name: "Card title", show_artwork: "Show artwork",
        show_remote: "Show directional remote", show_diagnostics: "Show diagnostics",
        show_favorites: "Show quick app buttons", show_activities: "Show activity selector",
  ...
  static getStubConfig(hass) {
    ...
    return {
      ...
      show_favorites: true, show_activities: true, confirm_power_off: true,
      confirm_restart: true, seek_seconds: 10, max_favorites: 6,
    };
  }
  ...
  setConfig(config) {
    ...
    this._config = {
      show_artwork: true, show_remote: true, show_diagnostics: false,
      show_favorites: true, show_activities: true, confirm_power_off: true,
      confirm_restart: true, seek_seconds: 10, max_favorites: 6, ...config,
    };
    this._signature = "";
    this._render();
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <ha-card><div class="card ${isOff ? "is-off" : ""}">
        ...
        ${this._config.show_diagnostics ? `<details class="diagnostics"><summary>${escapeHtml(labels.diagnostics)}</summary><dl><dt>${escapeHtml(labels.state)}</dt><dd>${escapeHtml(state.state)}</dd><dt>${escapeHtml(labels.health)}</dt><dd>${escapeHtml(health)}</dd><dt>${escapeHtml(labels.application)}</dt><dd>${escapeHtml(attrs.app_name || attrs.app_id || "—")}</dd><dt>${escapeHtml(labels.source)}</dt><dd>${escapeHtml(activeSource || "—")}</dd><dt>${escapeHtml(labels.volume)}</dt><dd>${volume}%${muted ? " · muted" : ""}</dd><dt>${escapeHtml(labels.features)}</dt><dd>${escapeHtml(attrs.supported_features ?? 0)}</dd></dl></details>` : ""}
      </div></ha-card>`;
    this._bindEvents();
  }
  ...
  async _call(domain, service, data) {
    ...
    catch (error) { console.error(`[Unified TV Card] ${domain}.${service} failed`, error); }
  }
...
if (!window.customCards.some((card) => card.type === CARD_TAG)) {
  window.customCards.push({
    ...
    description: "A compact TV, Cast, app, input, activity, playback and remote controller.",
    preview: true,
    ...
    getEntitySuggestion: (hass, entityId) => {
      ...
      return { config: { type: "custom:unified-tv-card", entity: entityId, show_artwork: true, show_remote: true, show_favorites: true, show_activities: true, confirm_power_off: true, confirm_restart: true, seek_seconds: 10 } };
    },
  });
}
